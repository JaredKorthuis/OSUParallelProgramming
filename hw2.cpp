...
main( int argc, char *argv[ ] )
{
	...
	for( int t = 0; t < NUMSTEPS; t++ )
	{
		...
		for( int i = 0; i < NUMBODIES; i++ )
		{
			...
			float fz = 0.;
			Body *bi = &Bodies[i];
        //    #pragma omp parallel for reduction(+:fx), reduction(+:fy), reduction(+:fz), schedule(static)
    ...
    double Mega_Bod = ((float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/1000000.);
    double times = (time1 - time0)*100;
    printf("NUMTHREADS: %d\n",NUMT);
    printf("NUMBODIES: %d\n",NUMBODIES);
    printf("NUMSTEPS: %d\n",NUMSTEPS);
    printf("Mega Bod/sec = %8.2f\n",Mega_Bod);
